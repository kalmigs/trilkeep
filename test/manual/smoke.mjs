...
async function main() {
  ...
  try {
    ...
    check('root title is v1', stampedNote?.title === rootTitleV1, `got "${stampedNote?.title}"`);
    ...
    check(
      ...
      rootLabels(await client.getNote(movedParent))[CONTAINER_PATH_LABEL] === `SmokeG2-${suffix}` &&
        (await parentOf(movedParent)) === 'root',
    );
    ...
    await new SyncEngine(client, manifest7b, roOpts, silentLog).backup(['note.md'], reporter);
    check(
    ...
    const roCount = ((await client.getNote(manifest7.rootNoteId))?.attributes ?? []).filter(
      a => a.type === 'label' && a.name === READONLY_LABEL,
    ).length;
    check(
    ...
    await new SyncEngine(client, manifest7, { ...roOpts, readOnly: false }, silentLog).backup(
      ['note.md'],
      reporter,
    );
    ...
    check(
      ...
      !(roRoot2?.attributes ?? []).some(a => a.type === 'label' && a.name === READONLY_LABEL),
    );
  } finally {
    ...
    for (const id of [rootNoteId, rootNoteId2, ...cleanupIds]) {
      if (id) {
        await client
          .deleteNote(id)
          .catch(e => console.log(`  (cleanup) could not delete note ${id}: ${e.message}`));
      }
    }
    for (const dir of [
      workspaceRoot,
